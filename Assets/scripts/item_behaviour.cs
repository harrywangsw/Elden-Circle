    ...
    void Update()
    {
        ...
        if((player.transform.position-transform.position).magnitude<=trigger_dist){
            ...
            if(Input.GetKeyDown(KeyCode.Return)&&message_screen.GetComponent<TMPro.TextMeshProUGUI>().text=="press enter to pick up item"){
                int i;
                for(i=0; i<plac.player_items.inv.Count; i++){
                    //Debug.Log(plac.player_items.inv[i].Item1);
                    if(plac.player_items.inv[i].Item1==gameObject.name){
                        plac.player_items.inv[i] = Tuple.Create(gameObject.name, plac.player_items.inv[i].Item2+1, gameObject.tag);
                        //Debug.Log(plac.player_items.inv[i].Item2.ToString());
                        plac.Update_quickslot();
                        Destroy(gameObject);
                        return;
                    }
                }
                plac.player_items.inv.Add(Tuple.Create(gameObject.name, 1, gameObject.tag));
